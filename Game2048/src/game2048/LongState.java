...
public class LongState {
    ...
    public int getColumn(int x) {
        ...
        return (int) (Masks.col0ToRow3(state >>> (x * Masks.SHIFT_COLUMN)) >>> (3 * Masks.SHIFT_ROW));
    }
    
    public int getColumnFlipped(int x) {
        assert x == (x & 3);
        
        return (int) (Masks.col0ToFlippedRow3(state >>> (x * Masks.SHIFT_COLUMN)) >>> (3 * Masks.SHIFT_ROW));
    }
    ...
    public void setColumnFlipped(int x, int value) {
        ...
        state &= ~((Masks.COLUMN_0 * Masks.MASK) << Masks.SHIFT_COLUMN * x);
        state |= column;
    }
